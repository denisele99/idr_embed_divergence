...
# Type of enrichment: knn, BLAST, random, 
# k = 50 default
...
def main_0():
    ...
    if go_annotation_path is None:
        raise ValueError("Config must contain 'go_annotations'.")
    ...
    if search_method == "knn":
        ...
        enrich = DistanceMatrix(dataset_path, go_annotation_dict)
        ...
        background_genes =list(set([_extract_gene_id(id) for id in distance_ids]))
        
        #Background genes = all genes in human proteome
        ...
        background_genes = [_extract_gene_id(id) for id in all_ids]

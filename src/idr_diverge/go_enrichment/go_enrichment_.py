...
def go_enrichment_BLAST_old(blast_res_path,k=50,alpha=0.05,outpath='out.csv', go_annotation_dict=GO_ANNOTATIONS): #TODO check if this works
    ...
    for query in query_ids:
        ...
        enriched_goterms, enriched_gocounts = _fisher(
...
def go_enrichment_random_old(query_ids, all_ids,k=50,alpha=0.05, go_annotation_dict = GO_ANNOTATIONS):
    """
    ...
    for query in query_ids:
        ...
        enriched_goterms,_ = go_enrichment_fisher(study_ids=NN_ids_short,
                            go_terms_background=go_background,
                            gene_id_background=list(go_background.keys()),
                            filter_alpha=alpha)
...
# ----------------------------
def go_enrichment_from_blast_old(
    blast_tsv_path: str,
    ...
    if background_genes is None:
        #background_genes = list(blast["query_gene"]) #allows duplicates
        background_genes = list(set(blast["query_gene"]))
        #background_genes = list(go_annotation_dict.keys()) #alternative method
        #background_genes = [_extract_gene_id(id) for id in distance_ids] #allows duplicates
        
    rows = []
    bg_term_counts = count_go_terms(background_genes, go_annotation_dict, deduplicate_genes=True)
    ...
    for q_id, sub in blast.groupby("query_id"):
        ...
        hits = [h for h in sub["hit_gene"].tolist() if h != q_gene][:k] #remove hits to self
        #hits = [h for h in sub["hit_id"].tolist() if h != q_id][:k]
        term_to_adj_p, term_to_counts = go_enrichment_fisher(
            study_genes=hits,
            background_genes=background_genes, #TODO THIS is wrong
            go_terms_by_id=go_annotation_dict, #TODO
            background_term_counts=bg_term_counts,
            alpha=alpha,
        )
        ...
        rows.append(
            {
                #"query_gene": q_gene,
                "query_id": q_id,
                "term_adj_p": term_to_adj_p,#list(term_to_adj_p.keys()),
                "term_counts": term_to_counts,
            }
        )

    # for q_gene, sub in blast.groupby("query_gene"):
    #     hits = [h for h in sub["hit_gene"].tolist() if h != q_gene][:k] #remove hits to self

    #     term_to_adj_p, term_to_counts = go_enrichment_fisher(
    #         study_ids=hits,
    #         background_ids=background_genes, #TODO THIS is wrong
    #         go_terms_by_id=go_annotation_dict, #TODO
    #         background_term_counts=bg_term_counts,
    #         alpha=alpha,
    #     )
...
def go_enrichment_random_old2(
    ...
    bg_term_counts = count_go_terms(bg, go_annotation_dict, deduplicate_genes=True)
    ...
    for q in query_ids:
        ...
        term_to_adj_p, term_to_counts = go_enrichment_fisher(
            study_genes=sample,
            background_genes=bg,
            go_terms_by_id=go_annotation_dict,
            background_term_counts=bg_term_counts,
            alpha=alpha,
        )
        #out[q] = list(term_to_adj_p.keys())
    ...
    if out_csv:
        df.to_csv(out_csv)
    return df
    #return pd.DataFrame.from_dict(out, orient="index", columns=["enriched_terms"])

...
import pandas as pd
import os
from typing import Dict, List, Mapping, Sequence, Optional, Tuple
...
def count_go_terms(
    ...
    ids: List[str],
    go_annotation_dict: Dict[str, List[str]],
    deduplicate_ids: bool = False
    #ids_are_segments: bool = True
) -> Counter[str]:
    ...
    """
    id_list = list(dict.fromkeys(ids)) if deduplicate_ids else list(ids)
    terms: List[str] = []
    for item_id in id_list:
        gene_id = _extract_gene_id(item_id)
        terms.extend(go_annotation_dict.get(gene_id, ()))

    return Counter(terms)

    

def prepare_go_enrichment(population_ids:List[str], 
                          obodag:GODag, 
                          go_annotation_dict:Dict[str, List[str]], 
                          alpha:float,
                        population_ids_are_segments:bool=True) -> GOEnrichmentStudy:
    """
    Prepare a GOATOOLS enrichment model.

    For IDR-level enrichment, population_ids should be unique segment/IDR IDs.
    Each segment is assigned the GO terms of its parent gene.

    Args:
        population_ids: Background IDs used as statistical units.
        obodag: GO DAG object from GOATOOLS.
        go_annotation_dict: Mapping from parent gene ID to GO term IDs.
        alpha: Significance threshold.
        population_ids_are_segments: If True, extract parent gene ID from each segment ID.

    Returns:
        Initialized GOEnrichmentStudy object.
    """
    #where population_genes is the background segment/idr set
    population_ids = list(set(population_ids))

    id_to_go_terms= {
        item_id: set(go_annotation_dict.get(_extract_gene_id(item_id) 
                                   if population_ids_are_segments else item_id,[]))
                    for item_id in population_ids}
    
    #id_to_go_terms = {seg: go_annotation_dict.get(seg, []) for seg in population_genes}
    
    return GOEnrichmentStudy(
        population_ids,
        id_to_go_terms, #gene_to_go_terms,
        obodag,
...
def run_go_enrichment(go_model:GOEnrichmentStudy, 
                      study_ids: List[str], 
                      alpha: float) -> Tuple[Dict[str, float], Dict[str, str]]:
    study_ids = list(dict.fromkeys(study_ids)) #deduplicate ids
    results = go_model.run_study(study_ids)
...
class DistanceMatrix:
    ...
    def __init__(self, embed_path, go_annotation_dict,go_obo_path ="/home/moseslab/denise/Paper/data/annotations/go_2024.obo"):  
        ...
        self.background_ids = self.distance_ids
        
        self.go_terms_background_count = count_go_terms(ids = self.background_ids, go_annotation_dict= go_annotation_dict)
    ...
    def find_kNN(self, query_ids: list, k=50):
        ...
        for query in present:
            ...
            NN_ids = [str(ids[i]) for i in neighbours]
    ...
    def go_enrichment_kNN(self, query_ids:List[str], k:int = 50, alpha:float =0.05):
        ...
        go_model = prepare_go_enrichment(population_ids = self.background_ids,
        ...
        for query, NN_ids in kNN_dict.items():
            ...
            #if query_gene in NN_ids: 
            #    NN_ids.remove(query_gene)
...
def go_enrichment_from_blast(
    ...
    if background_ids is None:
        background_ids = list(set(blast["query_id"])) #list(set(blast["query_gene"]))
        
    rows = []
    obodag = GODag(go_obo_path)
    go_model = prepare_go_enrichment(population_ids = background_ids,
    ...
    for q_id, sub in blast.groupby("query_id"):
        ...
        hits = [h for h in sub["hit_id"].tolist() if h != q_id][:k] #remove hits to self id (but not necessarily self gene)       
...
def go_enrichment_random(
    ...
    bg = list(background_ids)
    if len(bg) < k+1:
        raise ValueError(f"background_ids size ({len(bg)}) < k ({k})")
    
    obodag = GODag(go_obo_path)
    go_model = prepare_go_enrichment(population_ids = bg,
                                     go_annotation_dict=go_annotation_dict,
                                             obodag = obodag,
                                             alpha=alpha)
    ...
    for q in query_ids:
        ...
        eligible_bg = [background_id for background_id in bg if background_id != q]
        sample = rng.choice(eligible_bg, size=k, replace=False).tolist()
    ...
    df = pd.DataFrame(rows).set_index("query_id")  #make query_id the index?
    if out_csv:
        df.to_csv(out_csv)
    return df
